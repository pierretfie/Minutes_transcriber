...
### 3. Upload & Process Your Audio

Upload your audio file and choose a processing option. The upload page is split into two sections:

| Section | Option | Spec | When to use |
|---------|--------|------|-------------|
| **Compression** | No processing | Original codec / sample rate | Already compressed file |
|  | Standard (32 kbps) **(Recommended)** | 16kHz mono AAC 32k `ffmpeg -i in -ac 1 -ar 16000 -b:a 32k out.m4a` | Smaller file, clear recordings |
|  | High Clarity (16 kbps) | 16kHz mono AAC 16k | Noisy rooms, multiple speakers |
| **Denoised processing** | Denoise only | 48kHz mono Opus 64k `ffmpeg -i in -af arnndn=m=arnndn-models/std.rnnn -ac 1 -ar 48000 -c:a libopus -b:a 64k` | Remove fan/hiss, keep high quality |
|  | Compressed + Denoised **(Recommended)** | 24kHz mono Opus 32k `ffmpeg -i in -af arnndn=m=arnndn-models/std.rnnn -ac 1 -ar 24000 -c:a libopus -b:a 32k` | Best overall — noise removed + small file |

> **Denoising** uses RNNoise (`arnndn` filter, model `arnndn-models/std.rnnn` from [richardpl/arnndn-models](https://github.com/richardpl/arnndn-models)). Model path is configurable in **Settings → ARNNDN Model Path** (`config/settings.json: arnndnModelPath`, default `arnndn-models/std.rnnn`). The bundle includes the model. Silence removal (`silenceremove`) is intentionally not used.

If you prefer to process manually:

```bash
# Denoise only
ffmpeg -i input.m4a -af "arnndn=m=arnndn-models/std.rnnn" -ac 1 -ar 48000 -c:a libopus -b:a 64k denoised.opus

# Compressed + Denoised (Recommended)
ffmpeg -i input.m4a -af "arnndn=m=arnndn-models/std.rnnn" -ac 1 -ar 24000 -c:a libopus -b:a 32k denoised-compressed.opus
```
...
Once processed, you can download the resulting file. This means you don't have to re-process the same audio if you need to re-run a transcription later.
